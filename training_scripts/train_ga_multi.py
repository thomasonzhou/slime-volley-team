# Trains a multi-agent from scratch (no existing AI) using evolution
...
for tournament in range(1, total_tournaments+1):
  ...
  policy_left_2.set_model_params(population[m])
  policy_right_2.set_model_params(population[n])

  # the match between the mth and nth member of the population
  score, length = multi_rollout(env, policy_right, policy_left, policy_right_2, policy_left_2)
